...
class FileUpload extends React.Component<IProps, IState> {
    ...
    private renderUploadingAttachment = (attachment: UploadingInvoiceAttachment) => {
        ...
        return (
            <div key={attachment.fileName} className='invoice-attachment d-flex justify-content-between align-items-center mb-3'>
                <i className={'fa-2x ' + fileTypeIcon} />

                <div className='d-flex flex-column flex-grow-1 mx-4'>
                    <span>{attachment.fileName}</span>
                    { this.renderProgressBar(progress) }
                    <span>{ sizeText } ({progress}% is done)</span>
                </div>
                
                <button className='btn-link btn-invoice-delete' onClick={() => this.cancelUpload(attachment)}>
    ...
    private startUpload = (accepted: File[], rejected, event) => {
        ...
        for (const file of accepted) {
            ...
            axios(`${slug}/files/uploadfile`, {
                ...
                method: 'post',
                onUploadProgress: (progressEvent) => this.onUploadProgress(progressEvent, newAttachment.identifier)
            })
    ...
    private onUploadProgress = (progressEvent: ProgressEvent, identifier: string) => {
        ...
        if (index > -1) {
            attachmentsUploading[index].progress = (progressEvent.loaded / progressEvent.total) * 100;
    ...
    private cancelUpload = (attachment: UploadingInvoiceAttachment) => {
        ...
        const newAttachments = [
            ...attachmentsUploading.filter(a => a.identifier !== attachment.identifier),
        ];
    ...
    private onUploadComplete = (response: AxiosResponse, attachment: UploadingInvoiceAttachment) => {
        const { attachmentsUploading } = this.state;
