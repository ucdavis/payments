    ...
    public class PaymentsController : Controller
    {
        ...
        private async Task<PaymentEvent> ProcessPaymentEvent(ReceiptResponseModel response, Dictionary<string, string> dictionary)
        {
            ...
            var paymentEvent = new PaymentEvent
            {
                ...
                BillingFirstName  = response.Req_Bill_To_Forename.SafeTruncate(255),
                BillingLastName   = response.Req_Bill_To_Surname.SafeTruncate(255),
                BillingEmail      = response.Req_Bill_To_Email.SafeTruncate(500),
                BillingCompany    = response.Req_Bill_To_Company_Name.SafeTruncate(500),
                BillingPhone      = response.Req_Bill_To_Phone.SafeTruncate(255),
                BillingStreet1    = response.Req_Bill_To_Address_Line1.SafeTruncate(500),
                BillingStreet2    = response.Req_Bill_To_Address_Line2.SafeTruncate(500),
                BillingCity       = response.Req_Bill_To_Address_City.SafeTruncate(500),
                BillingState      = response.Req_Bill_To_Address_State.SafeTruncate(255),
                BillingCountry    = response.Req_Bill_To_Address_Country.SafeTruncate(255),
                BillingPostalCode = response.Req_Bill_To_Address_Postal_Code.SafeTruncate(255),
